...
GPU_Dev::GPU_Dev(void)
{
	...
	err = cudaGetDeviceCount(&this->devInf.deviceCount);
	if (err != cudaSuccess || this->devInf.deviceCount == 0)
	{
...
*/
int GPU_Dev::getDevInfoStr(int CUDA_device_ID, char *str, int strlen) 
{
	int dev;
	...
	snprintf(str, strlen,"Cuda device ID:%d, Device name:%s, supporting CUDA:%d.%d, \nmultiProcessorCount:%d, clockRate:%01.02f MHz, TotalMem:%01.02f MB\n",
