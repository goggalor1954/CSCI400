...
fp_digit zShortMod(z *u, fp_digit v)
{
	...
	q64 = (fp_word)u->val[u->size - 1] << BITS_PER_DIGIT;
	for (i=u->size - 2;i>=0;i--)
	{
		q64 |= u->val[i];
		q64 = (q64 % v) << BITS_PER_DIGIT;
	}

	return (fp_digit)(q64 >> BITS_PER_DIGIT);
...
{
	//numerical recipes code, section 20.6, page 919
	//designed for radix 256 elements :(
	int j,mn,nn=1;
	int su = u->size * 4;
	int sv = v->size * 4;
	int sw = su+sv;
	...
	mn = MAX(su, sv);
	while (nn<mn) nn <<= 1;	//find the smallest usable power of two for the transform
	nn <<= 1;
	a = malloc(nn * sizeof(double));
	if (a == NULL)
	{
		printf("error, could not allocate memory in fftmul\n");
		exit(1);
	}
	...
	for (j=0; j<su; j++)	//copy u to double precision array
	{
		k=j<<2;
		a[k+0] = (double)(u->val[j] & 0xFFFF);
		a[k+1] = (double)((u->val[j] & 0xFFFF0000) >> 16);
		a[k+2] = (double)((u->val[j] & 0xFFFF00000000) >> 32);
		a[k+3] = (double)((u->val[j] & 0xFFFF000000000000) >> 48);
	}
	for (; j < nn; j++)
		a[j] = 0.0;
	for (j=0; j<sv; j++)	//copy v to double precision array
	{
		k=j<<2;
		b[k+0] = (double)(v->val[j] & 0xFFFF);
		b[k+1] = (double)((v->val[j] & 0xFFFF0000) >> 16);
		b[k+2] = (double)((v->val[j] & 0xFFFF00000000) >> 32);
		b[k+3] = (double)((v->val[j] & 0xFFFF000000000000) >> 48);
	}
	for (; j < nn; j++)
		b[j] = 0.0;

	//perform the convolution, first the two transforms
	drealft(a-1,nn,1);
	drealft(b-1,nn,1);
	//then multiply the complex results
	b[0] *= a[0];
	b[1] *= a[1];
	for (j=2;j<nn;j+=2)
	{
	...
	//inverse transform
	drealft(b-1,nn,-1);
	//final pass for all the carries
	cy = 0.0;
	for (j = nn-1; j>=0; j--)
	{
		t = b[j]/(nn>>1) + cy + 0.5;	//0.5 allows for roundoff error
		cy = (fp_digit) (t/65536);
		b[j] = t - cy*65536;
	}
	if (cy >= 65536) printf("cannot happen in fftmul\n");
	
	//need to group 4 together.  endian-ness??
	for (j=1; j<sw; j++)
	{
		w[j] = (fp_digit)b[j-1];
	}
...
void zShiftRight32_1(z32 *out, z32 *in)
{	
	//Computes a = in >> 1 
	//only used in the trial division stage of QS
	...
	for (i = sb - 1; i >= 0; --i)
	{
		nextcarry = (in->val[i] & mask) << y;
		out->val[i] = in->val[i] >> 1 | carry;
		carry = nextcarry;
	}

	if ((out->val[sb-1] == 0) && (out->size > 1))
		out->size--;
