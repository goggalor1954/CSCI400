...
uint64 spSOE(uint64 *primes, uint64 lowlimit, uint64 *highlimit, int count)
{
	...
	*/
	...
	uint64 numflags, numbytes, numlinebytes, bucket_alloc = 0, large_bucket_alloc = 0;
	...
	if (sdata.pbound > 1000000)
	{
		...
		{
			if (locprimes[k] == 0)
				printf("ERROR: found prime == 0 in locprimes at location " PRIu64 "\n",k);
	...
	else
	{
		...
		if (VFLAG > 2)
		{
			...
			printf("allocating merge prime storage for " PRIu64 " primes\n",k);
		}
		locprimes = (uint64 *)realloc(locprimes,k * sizeof(uint64));
