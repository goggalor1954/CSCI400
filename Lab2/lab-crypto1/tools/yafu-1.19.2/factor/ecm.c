...
// declarations/definitions when using YAFU's ECM
	...
	//declarations/definitions when using GMP-ECM
	#include <gmp_xface.h>
	#include <ecm.h>
...
int ecm_loop(z *n, int numcurves, fact_obj_t *fobj)
{
	...
	int dummy = 0;
#else
	#error "unknown compiler for handling threads in ecm"
#endif
...
#elif defined(_MSC_VER)
	threads = (HANDLE *)malloc(THREADS * sizeof(HANDLE));
#endif
	...
	for (j=0; j < numcurves / THREADS; j++)
	{
		...
		for (i=0; i<THREADS; i++)
		{
			...
			pthread_create( &threads[i], NULL, 
						&ecm_do_one_curve, (void *)(&thread_data[i])); 
			
#elif defined(_MSC_VER)

			//from http://msdn.microsoft.com/en-us/library/ms682516(VS.85).aspx
			threads[i] = CreateThread( 
					NULL,                   // default security attributes
					0,                      // use default stack size  
					(LPTHREAD_START_ROUTINE)(&ecm_do_one_curve),  // thread function name
					(void *)(&thread_data[i]),  // argument to thread function 
