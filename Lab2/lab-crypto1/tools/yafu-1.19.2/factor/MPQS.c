...
static void sieve_block(uint8 *sieve, sieve_fb *fb, uint32 start_prime, uint8 s_init, fb_list_mpqs *fullfb);
static void test_block(uint8 *sieve, fb_element_mpqs *fb, uint32 start_prime);
static int check_relations_MPQS(uint32 sieve_interval, uint32 blocknum, uint8 sieve[] ,z *n, 
...
static void trial_divide_Q(z *Q,sieve_fb *fb,mpqs_rlist *full,mpqs_rlist *partial,
...
static void save_relation_mpqs(mpqs_rlist *list, uint32 offset, uint32 largeprime, uint32 num_factors, 
...
void MPQS(fact_obj_t *fobj)
{
	...
	if (VFLAG > 0)
		printf("\nstarting MPQS on c(%d): %s\n",ndigits(n),z2decstr(n,&gstr1));

	logprint(sieve_log,"starting MPQS on c(%d): %s\n",ndigits(n),z2decstr(n,&gstr1));
	...
	if (digits_n < 35)
		...
		sieve_params.num_blocks = 8;	//16
	else if (digits_n < 50)
		sieve_params.num_blocks = 16;
	else if (digits_n < 60)
	{
		sieve_params.num_blocks = 32;
		sieve_params.large_mult = 40;
	}
	else if (digits_n < 65)
	{
		sieve_params.num_blocks = 40;
	...
	{
		sieve_params.num_blocks = 64;
		sieve_params.large_mult = 60;
	}
	else if (digits_n < 75)
	{
		sieve_params.num_blocks = 80;
		sieve_params.large_mult = 70;
	}
	else if (digits_n < 80)
	{
		sieve_params.num_blocks = 100;
		sieve_params.large_mult = 80;
	}
	else
	{
		sieve_params.num_blocks = 100;
	...
	if (BLOCKSIZE == 32768)
		sieve_params.num_blocks *= 2;		//because values above are for 64k blocksize
	...
	*/
	...
	if (closnuf >= 70)
	{
		...
		start_prime = i;
		test_block(sieve,fb->list,start_prime);
		sum=0;
	...
	logprint(sieve_log,"==== sieve params ====\n");
	logprint(sieve_log,"factor base: %d primes (max prime = %u)\n",fb->B,pmax);
	logprint(sieve_log,"large prime cutoff: %u (%d * pmax)\n",cutoff,sieve_params.large_mult);
	logprint(sieve_log,"sieve interval: %d blocks of size %d\n",sieve_interval/BLOCKSIZE,BLOCKSIZE);
	logprint(sieve_log,"multiplier is %u\n",mul);
	if (smallprime)
		logprint(sieve_log,"small prime variation in effect, with initial value %d\n",small_bits);
	logprint(sieve_log,"trial factoring cutoff at %d bits\n",closnuf);
	logprint(sieve_log,"==== sieving started ====\n");
	...
	while (1)
	{
		...
		{	
			...
			sieve_block(sieve,fb_sieve_p,start_prime,blockinit,fb);

			i = check_relations_MPQS(sieve_interval,j2,sieve,n,poly,blockinit,fb_sieve_p,fb,full,partial,cutoff,small_bits,start_prime,0,&num,numpoly);

			sieve_block(sieve,fb_sieve_n,start_prime,blockinit,fb);

			i = check_relations_MPQS(sieve_interval,j2,sieve,n,poly,blockinit,fb_sieve_n,fb,full,partial,cutoff,small_bits,start_prime,1,&num,numpoly);
			...
			if (i >= check_total)
			{
				...
				if (VFLAG > 0)
				{
					for (i=0;i<(uint32)charcount;i++)
						printf("\b");
					charcount = printf("%d relations found: %d full + %d from %d partial, using %d polys",
						j+full->num_r,full->num_r,j,partial->num_r,numpoly);
					fflush(stdout);
				}
	...
	logprint(sieve_log,"%d relations found: %d full + %d from %d partial, using %d polys\n",partial->act_r+full->num_r,full->num_r,partial->act_r,partial->num_r,numpoly);
	...
	logprint(sieve_log,"Gauss elapsed time = %6.4f seconds.\n",t_time);
	logprint(sieve_log,"\n");
	logprint(sieve_log,"\n");
...
static int check_relations_MPQS(uint32 sieve_interval, uint32 blocknum, uint8 sieve[], z *n, mpqs_poly *poly, uint8 closnuf,
						...
						uint32 cutoff, uint8 small_cutoff, uint32 start_prime, uint32 parity, uint32 *num, int numpoly)
{
	...
	for (j=0;j<limit;j++)
	{
		...
		for (k=0;k<8;k++)
		{
			...
			trial_divide_Q(&Q,fb,full,partial,sieve,offset,8*j+k,neg,fullfb,cutoff,small_cutoff,start_prime,numpoly,parity,closnuf,sieve[8*j+k]);
		}
	}
	zFree(&Q);
	zFree(&t1);
						  ...
						  uint8 small_cutoff, uint32 start_prime, int numpoly, uint32 parity,uint8 closnuf,uint8 bits)
{
	...
	if ((Q->size == 1) && (Q->val[0] == 1))
	{
		...
		save_relation_mpqs(full,offset,1,smooth_num+1,num_f,fboffset,numpoly,parity);
	}
	else if ((Q->size == 1)  && (Q->val[0] < cutoff))
	{
		//printf("saving partial...\n");
		save_relation_mpqs(partial,offset,Q->val[0],smooth_num+1,num_p,fboffset,numpoly,parity);
...
{
	//check that b^2 == N mod a, that (d/N) = 1, that d mod 4 = 3,
	//and that c == (b*b - n)/a
	z t1,t2,t3,t4;
	...
	if (zJacobi(n,&poly->poly_d) != 1)
		printf("(d/N) != 1\n");

	if (zShortMod(&poly->poly_d,4) != 3)
		printf("d mod 4 != 3\n");
